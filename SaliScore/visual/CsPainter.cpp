...
CsPainter::CsPainter(QPainter *painter, const QString &keyViewSettings, const CsComposition &comp, const CsCursorPosition *player , int offsetX, QSize size, CsCellCursor *cellCursor) :
  ...
  mStepPixChord = visualX( 0, mStepChord = CS_STEP_CHORD );
  mStepPixNote  = visualX( 0, mStepNote = CS_STEP_NOTE );
  mStepPixLyric = visualX( 0, mStepLyric = CS_STEP_LYRIC );
  }
...
int CsPainter::drawLine(int y, int lineIndex, const CsLine &line, bool fullDrawing )
  ...
  if( line.isRemark() ) {
    //Remark only
    //Calculate visible count of remark
    int count = 0;
    for( const auto &def : mDefList )
      if( def.classGet() == cccRemark && def.mVisible ) count++;

    fullLineHeight = lineHeight = (mRemarkTextHeight.mHeight + mSettings.mTextGap) * count;
    }
  else {
    lineHeight = 0;
    for( const auto &def : mDefList )
      if( def.mVisible ) {
        switch( def.classGet() ) {
          case cccChord :
            //Chords
            lineHeight += (mChordTextHeight.mHeight + mSettings.mTextGap);
            break;
          case cccNote :
            //Notes
            lineHeight += 9 * mSettings.mScoreLineDistance;
            break;
          case cccLyric :
          ...
          case cccTranslation :
            //Translations
            lineHeight += (mTranslationTextHeight.mHeight + mSettings.mTextGap);
            break;
          }
        }
...
void CsPainter::drawLine(const CsLine &line)
  ...
  if( line.isRemark() ) {
    ...
    for( const auto &def : mDefList )
      if( def.mVisible && def.classGet() == cccRemark )
        drawRemarkLine( line, def );
    }
  else {
      ...
      if( def.mVisible ) {
        ...
        switch( def.classGet() ) {
          case cccChord :
            ...
            drawCellChord( mCurY, taktCount * mTickPerTakt, def.mName );
            mCurY += mChordTextHeight.mHeight;
            drawChordImpl( def.mName, line.chordListGet(def.mName) );
            mCurY += mSettings.mTextGap;
            break;

          case cccNote :
            //Draw note line
            drawCellNote( mCurY, taktCount * mTickPerTakt, def.mName );
            drawNoteImpl( def.mClef, taktCount, def.mName, line.noteListGet(def.mName) );
          ...
          case cccTranslation :
            ...
            if( mCellCursor != nullptr ) {
              int width = drawCellText( mLeftGap, mCurY + mTranslationTextHeight.mOffset, line.textGet(def.mName), mTranslationTextHeight.mHeight,
                            mCellCursor->isMatch( cccTranslation, mLineIndex, def.mName ) );
              mReferenceList.append( CsReference( mLeftGap, mCurY + mTranslationTextHeight.mOffset, width, mTranslationTextHeight.mHeight,
                                                  cccTranslation, mLineIndex, def.mName, 0 ) );
              }

            mCurY += mTranslationTextHeight.mHeight;
            drawTranslationImpl( mLeftGap, mCurY, line.textGet(def.mName) );
            mCurY += mSettings.mTextGap;
            break;
          }
        }
...
void CsPainter::drawRemarkLine(const CsLine &line, const CsDefinition &def)
  {
  //For each remark translations which visible we perform drawing
  if( mCellCursor != nullptr ) {
    int width = drawCellText( mLeftGap, mCurY + mRemarkTextHeight.mOffset, line.textGet(def.mName), mRemarkTextHeight.mHeight,
                  mCellCursor->isMatch( cccRemark, mLineIndex, def.mName ) );
    mReferenceList.append( CsReference( mLeftGap, mCurY + mRemarkTextHeight.mOffset, width, mRemarkTextHeight.mHeight,
                                        cccRemark, mLineIndex, def.mName, 0 ) );
    }

  mCurY += mRemarkTextHeight.mHeight;
  if( isNotEditRemark( def.mName, mLeftGap, mCurY ) )
    drawRemarkImpl( mLeftGap, mCurY, line.textGet(def.mName) );
  mCurY += mSettings.mTextGap;
  }
