...
CsPainterEditor::CsPainterEditor(QPainter *painter, const QString &keyViewSettings, const CsComposition &comp, const CsCursorPosition *player, int offsetX, QSize size, CsCellCursor *cellCursor, CsCursorEdit *cursorEdit, const QSet<int> selectedLines) :
...
bool CsPainterEditor::isNotEditProperty(const QString &propertyId, int x, int y)
  {
  if( mCursorEdit == nullptr || mCursorEdit->cellClass() != cccAttribute || mCursorEdit->partName() != propertyId )
    return true;
